...
// ----------------------------------------------------------------------------
void SocketTCP::createConnection(const char* server_name, unsigned int port) throw(NetworkError)
{
...
// ----------------------------------------------------------------------------
void SocketTCP::createTCPClient(in_port_t port, in_addr_t addr) throw(NetworkError)
{
    assert(!_est_init_tcp);
    if (!open()) {
...
// ----------------------------------------------------------------------------
void SocketTCP::createServer(in_port_t port, in_addr_t addr) throw(NetworkError)
{
    assert(!_est_init_tcp);
...
{
    G.Out(pdGendoc, "enter SocketTCP::send");
    ...
    G.Out(pdGendoc, "exit  SocketTCP::send");
}
