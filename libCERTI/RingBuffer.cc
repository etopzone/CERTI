                      ...
                      throw (certi::RingBufferNotCreated) 
{
	...
	#ifdef DEBUG
	std::cout << "RingBuffer::RingBuffer(..) - Constructor Call" << std::endl ;
	#endif
	...
	if(_Side == BUFFER_SC)
	{
		...
		#ifdef DEBUG
		std::cout << " RingBuffer::RingBuffer(..) - SC Segment Created " << std::endl ;
	...
	else
	{
		...
		#ifdef DEBUG
		std::cout << " RingBuffer::RingBuffer(..) - CS Segment Created " << std::endl ;
...
void RingBuffer::Attach() throw(certi::RingBufferNotAttached) 
{
	...
	if(_Side == BUFFER_CS)
	{
		...
		#ifdef DEBUG
		std::cout << " RingBuffer::Attach() - SC Segment Identified and Attached " << std::endl ;
		std::cout << " Adresse : _Shm_SC->GetShm() = " << _Shm_SC->GetShm() << std::endl ;
		std::cout << " Adresse : _Pw_Pr_SC->GetShm() = " << _Pw_Pr_SC->GetShm() << std::endl ;
		#endif
	}
	else
	{
		...
		#ifdef DEBUG
		std::cout << " RingBuffer::Attach() - CS Segment Identified and Attached " << std::endl ;
		std::cout << " Adresse : _Shm_CS->GetShm() = " << _Shm_CS->GetShm() << std::endl ;
		std::cout << " Adresse : _Pw_Pr_CS->GetShm() = " << _Pw_Pr_CS->GetShm() << std::endl ;
		#endif
	}
...
// ************************************************
void RingBuffer::Send( void *Buffer
					 , size_t Size
					 )
							...
							) 
{
	...
	#ifdef DEBUG
	std::cout << "RingBuffer::Send(..) - Function Start" << std::endl ;
	#endif

	if (Size > _Size) 
	{
		throw(certi::MessageTooLong("RingBuffer::Send() failed."));
		/* Should we return out of the function here? */
	}
	...
	if(_Side == BUFFER_SC)
	{
		...
		#ifdef DEBUG
		std::cout << "RingBuffer::Send(..) -> Start (Count_SC=" << _Tab_SC[0] << " | Write_SC=" << _Tab_SC[1] << " | Read_SC=" << _Tab_SC[2] << ")" << std::endl ;
		#endif

		if ((int)Size > _Tab_SC[0]) 
		{ // Test si il y a assez de place disponible dans le buffer (Exeption a envoyer)
			try 
			{
				_Sem_SC->V();
			}
		...
		} /* End of if ((int)Size > _Tab_SC[0]) */

		/* PRead >= PWrite || PWrite >= PRead && Size to Write + PWrite <= TotalSizeOfTheMemorySegment	*/
		if ( (_Tab_SC[2] > _Tab_SC[1]) || ((_Tab_SC[1] >= _Tab_SC[2]) && (Size + _Tab_SC[1] <= _Size))) 
		{
			...
			std::cout << "RingBuffer::Send(..) -> Adresse Utilisee  _Shm_SC->GetShm() = " 
		...
		else 
		{
			...
			memcpy( (void *)((char *)(_Shm_SC->GetShm())+ (sizeof(void *) * _Tab_SC[1]))
				  , Buffer
				  , Rest_byte_in_SHM 
				  );
			...
			_Tab_SC[1] = (_Tab_SC[1] + Size) % _Size;
			_Tab_SC[0] -= Size;
		} /* End of if/else ((_Tab_SC[2] >= _Tab_SC[1]) ...) */

		#ifdef DEBUG
		std::cout << "RingBuffer::Send(..) -> End (Count_SC="  << _Tab_SC[0] << " | Write_SC=" << _Tab_SC[1] << " | Read_SC =" << _Tab_SC[2] <<  ")" << std::endl ;
		#endif
	...
	else
	{
		...
		/* Test there is enough memory in the buffer */
		if ((int)Size > _Tab_CS[0]) 
		{ 
			try 
			{
			_Sem_CS->V();
		...
		{
			#ifdef DEBUG
			std::cout << "RingBuffer::Send(..) -> Use simple memcpy" << std::endl ;
		...
		{
			#ifdef DEBUG
			std::cout << "RingBuffer::Send(..) -> Use double memcpy" << std::endl ;
			#endif
	...
	#ifdef DEBUG
	std::cout << "RingBuffer::Send(..) - Function Completed" << std::endl ;
	#endif
...
// ************************************************
void RingBuffer::Receive( void *Buffer
						, size_t Size
						)
...
{
	#ifdef DEBUG
	std::cout << "RingBuffer::Receive(..) - Function Start" << std::endl ;
	#endif
	...
	if(_Side == BUFFER_SC)
	{
		...
		/* Test if there is enough space in the Buffer */
		...
		else
		{
			...
			{
				#ifdef DEBUG
				std::cout << "RingBuffer::Receive(..) -> Use simple memcpy" << std::endl ;
			...
			{
				#ifdef DEBUG
				std::cout << "RingBuffer::Receive(..) -> Use double memcpy" << std::endl ;
				#endif
			...
			std::cout << "Before memcpy(_Pw_Pr_CS->GetShm(), _Tab_CS, 3 * sizeof(int) ); !! " << std::endl ;
			#endif

			memcpy(_Pw_Pr_CS->GetShm(), _Tab_CS, 3 * sizeof(int) );

			#ifdef DEBUG
			std::cout << "After  memcpy(_Pw_Pr_CS->GetShm(), _Tab_CS, 3 * sizeof(int) ); !! " << std::endl ;
			#endif

		} /* End of if/else (_Tab_SC[0] == _Size ) */
	...
	else
	{
		...
		else 
		{
			...
			if ( (_Tab_SC[1] > _Tab_SC[2]) || ((_Tab_SC[2] >= _Tab_SC[1]) && (Size + _Tab_SC[2] <= _Size)) ) 
			{
				...
				#endif
				std::cout << "test 1 " << _Name << _Side << std::endl ;
				memcpy( Buffer
		...
		}  /* End of if/else (_Tab_CS[0] == _Size ) */
