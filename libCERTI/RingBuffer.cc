...
#ifdef DEBUG
        std::cout << " The SHMs for RingBuffer from SERVER to CUSTOMER Exchanges are Identified and Attached "
                  << std::endl;
        std::cout << " Adresse : _Shm_SC->GetShm() = " << _Shm_SC->GetShm() << std::endl;
        std::cout << " Adresse : _Pw_Pr_SC->GetShm() = " << _Pw_Pr_SC->GetShm() << std::endl;
#endif
...
#ifdef DEBUG
        std::cout << " The SHMs for RingBuffer from CUSTOMER to SERVER Exchanges are Identified and Attached "
                  << std::endl;
        std::cout << " Adresse : _Shm_CS->GetShm() = " << _Shm_CS->GetShm() << std::endl;
        std::cout << " Adresse : _Pw_Pr_CS->GetShm() = " << _Pw_Pr_CS->GetShm() << std::endl;
#endif
...
#ifdef DEBUG
        std::cout << "RingBuffer::Send(...) --> BEGIN Algorithm : Count_SC = " << _Tab_SC[0]
...
//  Plecture >= Pecriture || Pecriture >= Plecture && Taille a ecrire + Pecriture <=TailleTotaleSegment
            ...
            std::cout << "RingBuffer::Send(...) --> Adresse Utilisee  _Shm_SC->GetShm() = " << _Shm_SC->GetShm()
                      << "| _Shm_SC->GetShm() + _Tab_SC[1] = "
...
#ifdef DEBUG
        std::cout << "RingBuffer::Send(...) --> END of Algorithm : Count_SC = " << _Tab_SC[0]
                  << " | Write_SC = " << _Tab_SC[1] << "| Read_SC = " << _Tab_SC[2] << std::endl;
#endif
...
#ifdef DEBUG
        std::cout << "RingBuffer::Send(...) --> BEGIN Algorithm : Count_CS = " << _Tab_CS[0]
...
#ifdef DEBUG
            std::cout << "RingBuffer::Send(...) --> Utilisation memcpy Simple " << std::endl;
            std::cout << "RingBuffer::Send(...) --> Adresse Utilisee  _Shm_CS->GetShm() = " << _Shm_CS->GetShm()
                      << "| _Shm_CS->GetShm() + _Tab_CS[1] = "
...
#ifdef DEBUG
        std::cout << "RingBuffer::Send(...) --> END of Algorithm : Count_CS = " << _Tab_CS[0]
                  << " | Write_CS = " << _Tab_CS[1] << "| Read_CS = " << _Tab_CS[2] << std::endl;
#endif
...
void RingBuffer::Receive(void* Buffer, size_t Size)
{
    ...
    if (_Side == BUFFER_SC) {
        ...
        if (_Tab_CS[0]
            == (int) _Size) { // Test si il y a assez de place disponible dans le buffer (Exeption Ã  envoyer)
            try {
                _Sem_CS->V();
            }
            catch (Semaphore::SemaphoreHandlingError& e) {
...
#ifdef DEBUG
            std::cout << "RingBuffer::Receive(...) --> BEGIN Algorithm : Count_CS = " << _Tab_CS[0]
                      << " | Write_CS = " << _Tab_CS[1] << "| Read_CS = " << _Tab_CS[2] << std::endl;
#endif
...
//  Plecture >= Pecriture || Pecriture >= Plecture && Taille a ecrire + Pecriture <=TailleTotaleSegment
                ...
                std::cout << "RingBuffer::Receive(...) --> Adresse Utilisee  _Shm_CS->GetShm() = " << _Shm_CS->GetShm()
                          << "| _Shm_CS->GetShm() + _Tab_CS[2] = "
                          << (void*) ((char*) (_Shm_CS->GetShm()) + (sizeof(void*) * _Tab_CS[2])) << std::endl;
#endif
...
#ifdef DEBUG
            std::cout << "RingBuffer::Receive(...) --> END Algorithm : Count_CS = " << _Tab_CS[0]
                      << " | Write_CS = " << _Tab_CS[1] << "| Read_CS = " << _Tab_CS[2] << std::endl;
...
#ifdef DEBUG
            std::cout << "RingBuffer::Receive(...) --> Adresse _Tab_SC = " << _Tab_SC
                      << " Adresse _Pw_Pr_SC = " << _Pw_Pr_SC << std::endl;
            std::cout << "RingBuffer::Receive(...) --> Begin of Algorithm : Count_SC = " << _Tab_SC[0]
                      << " | Write_SC = " << _Tab_SC[1] << "| Read_SC = " << _Tab_SC[2] << std::endl;
#endif
...
#ifdef DEBUG
                std::cout << "RingBuffer::Receive(...) --> Utilisation memcpy Simple " << std::endl;
                std::cout << "RingBuffer::Receive(...) --> Adresse Utilisee  _Shm_SC->GetShm() = " << _Shm_SC->GetShm()
                          << "| _Shm_SC->GetShm() + _Tab_SC[2] = "
                          << (void*) ((char*) (_Shm_SC->GetShm()) + (sizeof(void*) * _Tab_SC[2])) << std::endl;
#endif
...
#ifdef DEBUG
            std::cout << "RingBuffer::Receive(...) --> End of Algorithm : Count_SC = " << _Tab_SC[0]
                      << " | Write_SC = " << _Tab_SC[1] << "| Read_SC = " << _Tab_SC[2] << std::endl;
#endif
