    ...
    ObjectClassHandle the_class, const std::string& theObjectName, FederationTime, FederationTime, Exception::Type& e)
{
    ...
    if (e == Exception::Type::NO_EXCEPTION) {
        auto responses = rootObject->registerObjectInstance(fm->my_federate_handle, the_class, rep->getObject(), rep->getLabel());
        std::cout << "==========================" << std::endl;
                                                        ...
                                                        Exception::Type& e)
{
    ...
    if (tm->testValidTime(theTime)) {
        ...
        rootObject->Interactions->isReady(fm->my_federate_handle, theInteraction, paramArray, paramArraySize);
                                                     ...
                                                     Exception::Type& e)
{
    ...
    if (e == Exception::Type::NO_EXCEPTION) {
        auto responses = rootObject->deleteObjectInstance(fm->my_federate_handle, theObjectHandle, theTag);
        std::cout << "==========================" << std::endl;
        std::cout << "RESPONSES FROM ROOT OBJECT" << std::endl;
        for (auto& rep : responses) {
            std::cout << (int) rep.message()->getMessageType() << std::endl;
...
void ObjectManagement::deleteAllObjects(Exception::Type& e)
{
    ...
    rootObject->getAllObjectInstancesFromFederate(fm->my_federate_handle, ownedObjectInstances);
