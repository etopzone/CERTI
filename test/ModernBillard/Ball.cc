 ...
 */
Ball::Ball(rti1516e::ObjectInstanceHandle h) : x(-10.0), y(-10.0), dx(3.0), dy(3.0), radius(10.0), active(false), ID(h)
{
...
void Ball::display()
{
    ...
    centre.X = (int) x;
    centre.Y = (int) y;
    disque = Definecr(centre, (int) radius, COUL_UNIE, active ? (couleur) color : GRAY);
    Drawcr(disque);
#endif
}
...
void Ball::setPosition(float xx, float yy)
{
    x = xx;
    y = yy;
}
...
void Ball::checkBorderCollision(float largeur, float hauteur)
{
 ...
 */
bool Ball::checkBallCollision(Ball* ab)
{
    return std::sqrt((x + dx - ab->x) * (x + dx - ab->x) + (y + dy - ab->y) * (y + dy - ab->y)) <= 2 * radius;
}

// ----------------------------------------------------------------------------
//! Initialize with a seed
void Ball::init(int seed)
{
    x = radius + seed * 60 + 3;
    y = radius + seed * 20;
