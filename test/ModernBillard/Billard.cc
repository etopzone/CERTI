...
Billard::Billard(std::string federate_name, rti1516e::RTIambassador& ambassador)
...
void Billard::join(std::string federation_name, std::string fdd_name)
{
    ...
    while (!joined && nb > 0) {
        nb--;
        try {
            handle = rtiamb.joinFederationExecution(my_federate_name.c_str(), federation_name.c_str(), this);
            joined = true;
            break;
        }
        catch (RTI::FederateAlreadyExecutionMember& e) {
            Debug(D, pdExcept) << "Federate " << my_federate_name << "already exists." << endl;

            throw;
        }
        catch (RTI::FederationExecutionDoesNotExist& e) {
            Debug(D, pdExcept) << "Federate " << my_federate_name << ": FederationExecutionDoesNotExist." << std::endl;
            // sleep(1);
        }
        catch (RTI::Exception& e) {
            Debug(D, pdExcept) << "Federate " << my_federate_name << " : Join Federation Execution failed : " << &e
                               << std::endl;
            throw;
        }
    }
...
void Billard::pause()
{
    if (creator) {
        ...
        try {
            ...
            Debug(D, pdExcept) << "Federate " << my_federate_name << " : Register Synchronization Point failed : " << endl;
        }
    }
}
