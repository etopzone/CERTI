...
void BillardStaticDDM::declare()
{
    ...
    GeoID = my_amb.getRoutingSpaceHandle("Geo");
    dimX = my_amb.getDimensionHandle("X", GeoID);
    dimY = my_amb.getDimensionHandle("Y", GeoID);

    std::cout << "Create regions";

    for (int i = 0; i < numberOfRegions; ++i) {
        // Subscription region
        RTI::Region* region = my_amb.createRegion(GeoID, 1);
        ...
        my_amb.notifyAboutRegionModification(*region);
        std::cout << ".";

        // Publication region
        region = my_amb.createRegion(GeoID, 1);
    ...
    local.ID = registerBallInstance(my_federate_name.c_str());
...
void BillardStaticDDM::checkRegions()
{
    ...
    if (region != subRegion || region != pubRegion) {
        ...
        // Subscription
        my_amb.subscribeObjectClassAttributesWithRegion(BilleClassID, *sub[region], *attributes, RTI::RTI_TRUE);
        if (subRegion != -1) {
            my_amb.unsubscribeObjectClassWithRegion(BilleClassID, *sub[subRegion]);
        }
        subRegion = region;

        // Update region
        my_amb.associateRegionForUpdates(*pub[region], local.ID, *attributes);
        if (pubRegion != -1) {
            my_amb.unassociateRegionForUpdates(*pub[pubRegion], local.ID);
        }
        pubRegion = region;
    }
}
...
void BillardStaticDDM::publishAndSubscribe()
{
    ...
    my_amb.publishObjectClass(BouleClassID, *attributes);
    my_amb.publishInteractionClass(BingClassID);
...
void BillardStaticDDM::resign()
{
    ...
    for (int i = 0; i < numberOfRegions; ++i) {
        my_amb.deleteRegion(sub[i]);
        my_amb.deleteRegion(pub[i]);
    }
    Billard::resign();
}
...
void BillardDynamicDDM::declare()
{
    ...
    my_amb.subscribeObjectClassAttributesWithRegion(BilleClassID, *region, *attributes, RTI::RTI_TRUE);

    my_amb.associateRegionForUpdates(*region, local.ID, *attributes);
}
...
{
    my_amb.unsubscribeObjectClassWithRegion(BilleClassID, *region);
    my_amb.unassociateRegionForUpdates(*region, local.ID);

    my_amb.deleteRegion(region);
